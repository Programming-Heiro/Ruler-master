...
// rayOrigin摄像头屏幕的位置   planeY为点击平面的位置
private func rayIntersectionWithHorizontalPlane(rayOrigin: SCNVector3, direction: SCNVector3, planeY: Float) -> SCNVector3? {
    let direction = direction.normalized()//direction->(x/sqrtf(x * x + y * y + z * z), y/sqrtf(x * x + y * y + z * z), z/sqrtf(x * x + y * y + z * z))
    //如果垂直平面距离为
    if direction.y == 0 {
...
extension ARSCNView {
    ...
    func worldPositionFromScreenPosition(_ position: CGPoint,
        ...
        // 过滤特征点，获取以摄像头和屏幕上一点对应的透视消失点为轴，角度为10度，最小距离为10cm，最大距离为三米组成的截锥体内的特征点
        let featureCloud = sceneView.fliterWithFeatures(highQualityfeatureHitTestResults)
        
        
        // 假设抽样的特征点符合正态分布，过滤偏差值大于3σ 的值
        if featureCloud.count >= 3 {
            
            // 根据特征点进行平面推定，根据过滤的点用最小二乘法进行平面推定
